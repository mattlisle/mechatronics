...
XT_Wav_Class Song(FullTrap);     // create an object of type XT_Wav_Class that is used by 
...
void loop() {
  ...
  if(Song.Completed){          // if completed playing, play again
    DacAudio.PlayWav(&Song);  // play the wav (pass the wav class object created at top of code
  }
  Serial.println(i);                  // print out the value of i
  i++;                                // increment the value of i

  if(digitalRead(36) == HIGH){
     DacAudio.PlayWav(&Song);
  }
}
